...
/**
 * 添加自定义质押合约。
 * body: { chain_id, platform, pair_label?, contract, read_type? }
...
export async function POST(req: Request) {
  const b = await getBody<{ chain_id?: number; platform?: string; pair_label?: string; contract?: string; read_type?: string }>(req);
  ...
  try {
    const info = db.prepare(
      `INSERT INTO staking_contracts (chain_id_ref, platform, pair_label, contract, read_type, enabled)
       VALUES (?,?,?,?,?,1)`
    ).run(b.chain_id, b.platform.trim(), b.pair_label?.trim() ?? "", b.contract.toLowerCase(), b.read_type?.trim() || "deposits_owner");
